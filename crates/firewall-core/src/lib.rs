...
///
/// This is a superset of `init()` — `init()` calls this with `FirewallProfile::Default`.
/// Safe to call multiple times — returns cached result after the first call.
/// Profile cannot be changed after the first successful init.
///
/// # Safety Actions
