...
//
// Safety architecture (IEC 61508 §6.7.3):
...
//
// Diagnostic Coverage target: ≥ 90% (DC_medium per IEC 61508 Table A.14).
        ...
        match (&a.decision, &b.decision) {
            ...
            (ChannelDecision::Pass { intent: ia }, ChannelDecision::Pass { intent: ib }) => {
                if ia == ib {
                    ...
                    // Intent disagreement — still Pass but diagnostic event raised.
                    // The operator must review DiagnosticAgreement events regularly
                    // (Safety Requirement SR-007: review within 24 h).
